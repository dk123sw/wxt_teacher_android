...
public class AlbumDetailActivity extends Activity {
    ...
    private TextView homework_content,teacher_name,homework_time,homework_item_praise_names,alread_text,not_read_text,top_title;
            ...
            switch (msg.what) {
                    ...
                    if (msg.obj != null) {
                        try {
                            ...
                            if (state.equals(CommunalInterfaces._STATE)) {
                                new ClassCircleRequest(context, handler).getCircleList(null, "1", "1", "2", GET_CIRCLE_LIST_KEY);
                            }
                ...
                case CommunalInterfaces.SEND_PARENT_REMARK:
                    ...
                    if (msg.obj !=null){
                        ...
                        Log.d("是否成功", state);
                        if (state.equals(CommunalInterfaces._STATE)){
                            Toast.makeText(AlbumDetailActivity.this, "发送成功", Toast.LENGTH_SHORT).show();
                            parent_warn_comment_edit.setText(null);
                            new ClassCircleRequest(context, handler).getCircleList(null, "1", "1", "2", GET_CIRCLE_LIST_KEY);
                        } else {
                            Toast.makeText(AlbumDetailActivity.this, "发送失败", Toast.LENGTH_SHORT).show();
                        }
    ...
    private void initData() {
        ...
        homeworkData =(ClassCricleInfo)intent.getSerializableExtra("album");
        user = new UserInfo(context);
        user.readData(context);
    }
    private void initview() {
        top_title = (TextView) findViewById(R.id.top_title);
        top_title.setText("相册详情");
        ...
        btn_parent_send.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (parent_warn_comment_edit.getText().length() > 0) {
                    ...
                    new NewsRequest(context, handler).send_remark(homeworkData.getId(), String.valueOf(parent_warn_comment_edit.getText()), "2");
...
//                holder.img_gridview.setVisibility(View.GONE);
    ...
    private void workPraise(String workBindId) {
        new NewsRequest(context, handler).Praise(workBindId, WORK_PRAISE_KEY,"2");
    }

    // 取消点赞
    private void delPraise(String workBindId) {
        new NewsRequest(context, handler).DelPraise(workBindId, DEL_WORK_PRAISE_KEY,"2");
    }
    /**
     * 返回 普通资讯 JSON
     *
